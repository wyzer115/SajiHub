...
@section('title', 'Edit Admin Cabang - SajiHUB')
...
<div class="max-w-2xl mx-auto animate-fade-in-up space-y-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-black text-[#8C0000]">Edit Admin Cabang: {{ $user->name }}</h2>
            <p class="text-slate-600 text-xs font-medium">Perbarui data akun Admin Cabang. Kosongkan password jika tidak diubah.</p>
        </div>
    </div>
    ...
    <div class="bg-white border border-stone-200 rounded-3xl p-6 shadow-sm">
        ...
        <form action="{{ route('superadmin.users.update', $user->id) }}" method="POST" class="space-y-5">
            ...
            <p class="text-xs text-slate-500 font-medium">Kosongkan field password jika tidak ingin mengubah password.</p>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-bold text-stone-700 uppercase tracking-wider mb-2">Password Baru (Opsional)</label>
                    <input type="password" name="password" minlength="6"
                           class="w-full bg-stone-50 border border-stone-300 text-[#1C1917] font-semibold rounded-xl px-4 py-2.5 text-sm focus:border-[#BD2000] focus:outline-none transition-all"
                           placeholder="Biarkan kosong jika tidak diubah">
                </div>
                <div>
                    <label class="block text-xs font-bold text-stone-700 uppercase tracking-wider mb-2">Konfirmasi Password Baru</label>
