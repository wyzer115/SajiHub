...
@section('title', 'Tambah Barang Inventaris - SajiHUB')
...
<div class="max-w-2xl mx-auto animate-fade-in-up">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h2 class="text-xl font-black text-[#8C0000]">Tambah Barang / Stok Baru</h2>
            <p class="text-slate-600 text-sm font-medium">Tambahkan bahan makanan, minuman, atau peralatan cabang</p>
        </div>
    </div>
    ...
    <div class="bg-white border border-stone-200 rounded-3xl p-6 shadow-sm">
        ...
        <form action="{{ route('supervisor.inventory.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required
                           ...
                           placeholder="Contoh: Daging Ayam, Beras, Wajan">
                </div>
                ...
                <div>
                    ...
                    <select name="category" required class="w-full bg-stone-50 border border-stone-300 text-[#1C1917] font-semibold rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#BD2000]">
                        <option value="bahan_makanan" {{ old('category') == 'bahan_makanan' ? 'selected' : '' }}>Bahan Makanan</option>
                        <option value="bahan_minuman" {{ old('category') == 'bahan_minuman' ? 'selected' : '' }}>Bahan Minuman</option>
                        <option value="peralatan" {{ old('category') == 'peralatan' ? 'selected' : '' }}>Peralatan / Alat Masak</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                          class="w-full bg-stone-50 border border-stone-300 text-[#1C1917] font-semibold rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#BD2000]"
                          placeholder="Catatan merek, supplier, atau kondisi barang...">{{ old('notes') }}</textarea>
            </div>
