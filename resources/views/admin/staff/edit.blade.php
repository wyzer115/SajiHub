...
<div class="max-w-3xl mx-auto space-y-6 animate-fade-in-up">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-black text-[#8C0000]">Edit Staff: {{ $user->name }}</h2>
    ...
    <div class="bg-white border border-stone-200 rounded-3xl p-8 shadow-sm">
        ...
        <form action="{{ route('admin.staff.update', $user->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                <label class="block text-xs font-bold text-stone-700 uppercase tracking-wider mb-2">Email <span class="text-red-500">*</span></label>
                <input type="email" name="email" value="{{ old('email', $user->email) }}" required
                       class="w-full bg-stone-50 border border-stone-300 text-[#1C1917] font-semibold rounded-xl px-4 py-3 text-sm focus:border-[#BD2000] focus:outline-none transition-all">
            </div>

            <div class="h-px bg-stone-200 my-1"></div>
            <p class="text-xs text-slate-500 font-medium">Kosongkan field password jika tidak ingin menggantinya.</p>
